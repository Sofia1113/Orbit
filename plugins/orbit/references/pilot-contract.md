...
- 所有 Orbit 工件必须写入 `/orbit:pilot` 启动仓库根目录的 `.orbit/state/<task_id>/`；`.orbit/<task_id>/`、仓库根目录、任务实现子目录下的 `.orbit/` 或其他路径都无效。
...
- medium 最少写入：`.orbit/state/<task_id>/triage.md`、`scope.md`、`plan.md`、`task_packet.json`、`execution.md`、`verification.md`、`runtime.json`。
- high 最少写入：`.orbit/state/<task_id>/triage.md`、`design.md`、`plan.md`、`task_packet.json`、`execution.md`、`verification.md`、`review.md`、`runtime.json`；`design.md` 必须来自或包含 `architect` 的方案设计，`review.md` 必须包含 architecture、spec-compliance 与 code-quality 三个 verdict。
...
- [ ] 若 density 为 medium：已完成 discovery、scoping、planning、一个或多个 low 子任务 execute/verify、父任务 integration verify，并在 `.orbit/state/<task_id>/` 写入 `scope.md` / `plan.md` / `task_packet.json` / `execution.md` / `verification.md` / `runtime.json`。
- [ ] 若 density 为 high：进入 design 前已按 `engine-high.md` 处理 worktree 决策与 brainstormer 五阶段确认；若用户批准继续，已完成 architect design、planning、一个或多个 medium 子任务 integration verify、父任务端到端 integration verify、architecture review、spec-compliance review 与 code-quality review。
...
- [ ] 所有 artifact 路径符合标准格式 `.orbit/state/<task_id>/<file>`，子任务路径包含父 ID 与序数后缀。
