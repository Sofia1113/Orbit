...
12. 未经用户批准，不得进入 planning。approval gate 必须调用 `AskUserQuestion`，且调用参数必须包含顶层 `questions: [...]`；每个 question 必须包含 `header`、`question`、`options`。使用 preview 模式时，`preview` 只能作为 `options[]` 条目的附加字段展示方案摘要，不能作为顶层字段，不能省略 `questions`。
