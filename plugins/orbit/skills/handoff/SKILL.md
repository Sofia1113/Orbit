...
用户体验目标：让下一位接手者不用重读全程也能继续。handoff.md 面向人类，应该像接力单；handoff.json 面向机器，必须保持最小、结构化、可校验。
...
`handoff.md` 应保持短而可执行，推荐包含：

- 当前焦点：正在处理的 step / subtask / blocker
- 已确认决策：用户批准的方案、明确的 scope、不可更改的约束
- 关键事实：已改文件、已跑验证、evaluator 结论或失败证据
- 风险与待验证项：下一轮必须先看的问题
- 下一步唯一动作：具体到 skill / agent / 命令或需要用户回答的问题
