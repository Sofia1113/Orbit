...
- **任务清单进入项**（用 `TaskCreate`）：澄清范围、候选方案枚举、推荐方案与取舍
...
- **`Plan` agent**（推荐）：当候选方案涉及跨模块协议、抽象层选择或潜在迁移成本时，dispatch 独立 Plan agent 做架构推演，得到第二意见再融入 options。Plan agent 比主会话更适合产出"实现策略 + 关键文件 + 架构权衡"的结构化结论
