...
- **TodoWrite 进入项**：checks[] 展开为 items
...
| `FAIL` | 声明 `VERIFY_FAIL`；`repair_actions` 逐条写入 TodoWrite；交回 `first_executor`；`REPAIR_SUBMITTED` 后 `verify_fail_streak` +1 |
