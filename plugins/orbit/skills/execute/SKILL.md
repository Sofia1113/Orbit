---
name: execute
description: Orbit 实现执行阶段。任务边界已收敛、准备写代码时调用：按 task_packet 推进实现，TodoWrite 同步进度，FAIL 时以 first_executor 身份进入 repairing。不自宣完成，结论交给 verify。
---
...
5. `action_updates`：TodoWrite 与 runtime.todo[] 同步结果
...
- [ ] TodoWrite 已同步（所有实现类 todo 为 done，未完成项挂到 handoff）
